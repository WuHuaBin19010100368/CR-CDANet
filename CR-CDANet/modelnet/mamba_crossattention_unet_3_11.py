...
class TimeEmbedding(nn.Module):
    def __init__(self, n_embd):
        """
        初始化 TimeEmbedding 模块。

        参数:
        n_embd (int): 输入的时间信息的维度。
        """
        super().__init__()
        # 第一个线性层，将输入维度 n_embd 映射到 4 * n_embd
        self.linear_1 = nn.Linear(n_embd, 4 * n_embd)
        # 第二个线性层，保持输出维度为 4 * n_embd
        self.linear_2 = nn.Linear(4 * n_embd, 4 * n_embd)

    def forward(self, x):
        """
        前向传播方法，将时间信息从低维映射到高维并引入非线性。

        参数:
        x (torch.Tensor): 输入的时间信息张量，形状为 (1, n_embd)。

        返回:
        torch.Tensor: 处理后的时间嵌入张量，形状为 (1, 4 * n_embd)。
        """
        # 输入形状: (1, 320)
        # 通过第一个线性层将输入维度从 n_embd 映射到 4 * n_embd
        x = x.squeeze(1)
...
class VSSBlock_x_t(nn.Module):
    def __init__(
        ...
        # 添加 UNET_ResidualBlock 和 UNET_AttentionBlock
        self.unet_residual_block = UNET_ResidualBlock(in_channels=hidden_dim, out_channels=hidden_dim, n_time=n_time)
        self.first_block = first_block

    def forward(self, input: torch.Tensor, time: torch.Tensor):
        # 先通过 UNET_ResidualBlock 和 UNET_AttentionBlock，仅在第一个块中执行
        if self.first_block:
            input_1 = input.permute(0, 3, 1, 2)
            x = self.unet_residual_block(input_1, time)
            x1 = x.permute(0, 2, 3, 1)
            # print('噪声第一层')
        else:
            x1 = input
            # print('噪声第二层')
        # print('测试测试测试', x1.shape)
        # 最后通过原始的 VSSBlock
        x2 = x1 + self.drop_path(self.self_attention(self.ln_1(x1)))
...
class VSSM(nn.Module):
    ...
    def forward_features(self, x, time, context):
        skip_list = []
        context_features = []  # 用于存储条件编码器的中间特征
        # 输入: (B, C, H, W) -> 输出: (B, H/4, W/4, embed_dim)
        ...
        for i, (layer_context, layer_x_t) in enumerate(zip(self.layers_context, self.layers_x_t)):
            ...
            if i == 0:
                ...
                combined = self.shallow_fusion1_64(torch.concat([context, x], dim=1)) + context
                ...
                context_features.append(context)  # 将特征图转换回 b*h*w*c 格式
            ...
            # if base_filter == 128:
            #     combined = self.shallow_fusion1_128(torch.concat([context, x], dim=1)) + context
            #     combined = self.shallow_fusion2_128(torch.concat([combined, x], dim=1)) + combined
            # elif base_filter == 256:
            #     combined = self.shallow_fusion1_256(torch.concat([context, x], dim=1)) + context
        ...
        return combined, skip_list, context_features
    ...
    def forward(self, x, context, time):
        ...
        x, skip_list, context_features = self.forward_features(x, time, context)
        ...
        # 返回最终预测结果
        return x, layer_outputs, context_features
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, noise_level):
        step = torch.arange(self.count, dtype=noise_level.dtype,
                            device=noise_level.device) / self.count
...
"""
VMUNet 类是整个网络的顶层封装，它调用了 VSSM 实现的编码器和解码器，并在 forward 方法中完成了整个前向传播过程。
"""
class VMUNet(nn.Module):
    def __init__(self,
        ...
        out_channels = 320  # 直接输出 320 维
        self.noise_level_mlp = nn.Sequential(
            PositionalEncoding(out_channels),
        ...
        self.time_embedding = TimeEmbedding(n_embd=out_channels)

    def forward(self, x_xnoisy, timesteps):
        ...
        logits, layer_outputs, context_features = self.vmunet(x, context, time_emb)

        return logits, context_features
...
if __name__ == "__main__":
    # 示例使用
    context = torch.randn(2, 3, 32, 32).to("cuda:1")
    x = torch.randn(2, 3, 32, 32).to("cuda:1")
    x_xnoisy = [context, x]
    net = VMUNet().to("cuda:1")
    timesteps = torch.randn(2, 1).to("cuda:1")
    aaa, ccc = net(x_xnoisy, timesteps)
    print(aaa.shape)
    print(ccc[0].shape), print(ccc[1].shape), print(ccc[2].shape)
