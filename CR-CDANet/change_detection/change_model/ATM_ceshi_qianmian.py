...
class VSSBlock_context(nn.Module):
    ...
    def forward(self, context: torch.Tensor):
        ...
        # 最后通过原始的 VSSBlock
        # print('x1:',x1.shape)
        # print('ln_1(x1):',self.ln_1(x1).shape)
        # print('self_attention：',(self.self_attention(self.ln_1(x1))).shape)
        # print('drop_path：',(self.drop_path(self.self_attention(self.ln_1(x1)))).shape)
...
class VSSM(nn.Module):
    ...
    def forward_features(self, x, context):
        ...
        encoder_outputs_t1 = []  # 用于存储每一层编码器的输出
        encoder_outputs_t2 = []  # 用于存储每一层编码器的输出
        ...
        for i, (layer_context, layer_x_t) in enumerate(zip(self.layers_context, self.layers_x_t)):
            ...
            if i == 0:
                ...
                encoder_outputs_t1.append(context)
                encoder_outputs_t2.append(x)
            ...
            # 返回最终特征和跳跃连接特征列表
        return combined, skip_list, encoder_outputs_t1, encoder_outputs_t2
    ...
    def forward(self, x, context):
        # 输入: (B, C, H, W) -> 输出: (B, H/(4*2^(num_layers-1)), W/(4*2^(num_layers-1)), dims_decoder[-1]), 跳跃连接特征列表
        x, skip_list, encoder_outputs_t1, encoder_outputs_t2= self.forward_features(x, context)
        # 输入: (B, H/(4*2^(num_layers-1)), W/(4*2^(num_layers-1)), dims_decoder[-1]), 跳跃连接特征列表 -> 输出: (B, H, W, dims_decoder[-1]//4)
        layer_outputs = self.forward_features_up(x, skip_list)
        # 返回最终预测结果
        return layer_outputs, encoder_outputs_t1, encoder_outputs_t2
...
class ATMamba(nn.Module):
    ...
    def forward(self, modal1 , modal2):
        ...
        layer_outputs_1, encoder_outputs_m1_t1, encoder_outputs_m1_t2 = self.vmunet_m1(modal1_T1, modal1_T2)
        layer_outputs_2, encoder_outputs_m2_t1, encoder_outputs_m2_t2 = self.vmunet_m2(modal2_T1, modal2_T2)
        # print(len(layer_outputs_1), layer_outputs_1[0].shape, layer_outputs_1[1].shape, layer_outputs_1[2].shape)
        logits= self.fusion(layer_outputs_1, layer_outputs_2)

        if self.num_classes == 1:
            return torch.sigmoid(logits), encoder_outputs_m1_t1, encoder_outputs_m1_t2,encoder_outputs_m2_t1, encoder_outputs_m2_t2
        else:
            return logits, encoder_outputs_m1_t1, encoder_outputs_m1_t2,encoder_outputs_m2_t1, encoder_outputs_m2_t2
...
if __name__ == "__main__":
    ...
    context2 = torch.randn(128, 3, 16, 16).to("cuda:1")
    x2 = torch.randn(128, 3, 16, 16).to("cuda:1")
    x_xnoisy = [context, x]
    x_xnoisy2 = [context2, x2]
    net = ATMamba().to("cuda:1")
    result, encoder_outputs_m1_t1, encoder_outputs_m1_t2, encoder_outputs_m2_t1, encoder_outputs_m2_t2=net(x_xnoisy, x_xnoisy2)
    print(result.shape)
    print(len(encoder_outputs_m1_t1),len(encoder_outputs_m1_t2),len(encoder_outputs_m2_t1),len(encoder_outputs_m2_t2))
    print(encoder_outputs_m1_t1[0].shape, encoder_outputs_m1_t2[0].shape,encoder_outputs_m2_t1[0].shape, encoder_outputs_m2_t2[0].shape)
    print(encoder_outputs_m1_t1[1].shape, encoder_outputs_m1_t2[1].shape,encoder_outputs_m2_t1[1].shape, encoder_outputs_m2_t2[1].shape)
    print(encoder_outputs_m1_t1[2].shape, encoder_outputs_m1_t2[2].shape,encoder_outputs_m2_t1[2].shape, encoder_outputs_m2_t2[2].shape)
